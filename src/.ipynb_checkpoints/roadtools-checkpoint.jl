...
"""
    links2graph(dfL::DataFrame) -> SimpleGraph

Convert a links DataFrame to a Graphs.jl SimpleGraph.

# Arguments
- `dfL`: Links DataFrame with SRC and DST columns (first two columns).

# Returns
- SimpleGraph with edges from the links.

# Example
```julia
dfL, dfN = faf5links(), faf5nodes()
dfL, dfN = prune_reindex(dfL, dfN)  # Ensure sequential node IDs
g = links2graph(dfL)
```
"""
function links2graph(dfL::DataFrame)
    max_node = max(maximum(dfL[:, 1]), maximum(dfL[:, 2]))
    g = Graphs.SimpleGraph(max_node)
    for row in eachrow(dfL)
        Graphs.add_edge!(g, row[1], row[2])
    end
    return g
end
