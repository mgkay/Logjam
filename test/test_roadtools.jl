...
@testset "RoadTools" begin
    ...
    # ─── H5: adj2graph ─────────────────────────────────────────────
    @testset "adj2graph" begin
        @testset "directed" begin
            g = adj2graph([0 5; 3 0])
            @test Graphs.nv(g) == 2
            @test Graphs.has_edge(g, 1, 2)
            @test Graphs.has_edge(g, 2, 1)
            @test g.weights[2, 1] ≈ 5.0  # 1→2
            @test g.weights[1, 2] ≈ 3.0  # 2→1
        end

        @testset "undirected" begin
            g = adj2graph([0 5; 0 0]; directed=false)
            @test Graphs.has_edge(g, 1, 2)
            @test Graphs.has_edge(g, 2, 1)
            @test g.weights[2, 1] ≈ 5.0
            @test g.weights[1, 2] ≈ 5.0
        end

        @testset "non-square error" begin
            @test_throws ArgumentError adj2graph([0 5 3; 1 0 2])
        end
